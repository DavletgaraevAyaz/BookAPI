...
public class BooksService : IBooksService  // Реализуем интерфейс IBooksService
{
    ...
    public async Task<IEnumerable<Books>> SearchBooksAsync(string author, string genre, int? year)
    {
        ...
        {
            query = query.Where(b => b.Genre_ID.ToString().Contains(genre));
        }
    ...
    public async Task<IEnumerable<Books>> GetMostPopularBooksAsync(int count)
    {
        return await _context.Books
            .Include(b => b.Genres)
            .OrderByDescending(b => _context.Rentals.Count(r => r.Books_Id == b.Id_Books))
            .Take(count)
            .ToListAsync();
    }
    ...
    public Task<Books> GetBookssByIdAsync(int id)
    {
        throw new NotImplementedException();
    }

    public Task AddBookssAsync(Books newBookss)
    {
        throw new NotImplementedException();
    }

    public Task UpdateBookssAsync(int id, Books updatedBookss)
    {
        throw new NotImplementedException();
    }

    public Task DeleteBookssAsync(int id)
    {
        throw new NotImplementedException();
    }
}
